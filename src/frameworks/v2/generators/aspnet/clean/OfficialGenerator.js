...
class AspNetCleanOfficialGenerator extends BaseFrameworkGenerator {
  constructor(runtime) {
    super({ frameworkId: 'aspnet', architectureId: 'clean', lang: 'csharp', port: DEFAULT_PORT });
  ...
  async generate(config, deps = {}) {
    ...
    try {
      ...
      if (!fs.existsSync(readmePath)) {
        fs.writeFileSync(readmePath, README(projectName), 'utf8');
      ...
      const { written: devopsWritten } = writeDevOpsFiles(devopsFiles, projectPath);
      filesWritten.push(...devopsWritten);
